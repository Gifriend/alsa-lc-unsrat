        ...
        {isLoading ? (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
            {[1, 2, 3].map((i) => (
              <div
                key={i}
                className="bg-gray-100 h-96 rounded-xl animate-pulse"
              ></div>
            ))}
          </div>
                  ...
                  <div className="flex justify-between items-center mt-auto pt-4 border-t border-gray-100">
                    ...
                    {item.whatsappNumber ? (
                      <a
                        href={`https://wa.me/${
                          item.whatsappNumber
                        }? text=Halo, saya ingin memesan ${encodeURIComponent(
                          item.name
                        )}`}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="bg-background text-primary px-5 py-2.5 rounded-lg text-sm font-semibold hover:bg-background/70 transition-colors shadow-sm inline-block text-center"
                      >
                        Order Now
                      </a>
                    ) : (
                      <button
                        disabled
                        className="bg-gray-300 text-gray-500 px-5 py-2.5 rounded-lg text-sm font-semibold cursor-not-allowed"
                      >
                        Contact Unavailable
                      </button>
                    )}
