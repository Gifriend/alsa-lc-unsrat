...
export async function GET() {
  try {
    const snapshot = await adminDb.collection("proker").orderBy("startDate", "desc").get()
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const docRef = await adminDb.collection("proker").add({
      ...body,
      createdAt: new Date(),
      updatedAt: new Date(),
    })
    ...
    return Response.json({ message: "Error creating proker" }, { status: 500 })
  }
}
